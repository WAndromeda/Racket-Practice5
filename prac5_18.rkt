...
(define (checkCode text)
   ...
   (cond [(not(eq? (string-length id) 0))
     (cond [(eq? (member id langs) #f)
            ...
            (cond [(string=? assign ":=" )
                   (set! lastWordPos pos)
                   (define val (getExpr text 0 (list "(" "not" "true" "false" "id") #f) )
                   (cond [(eq? error #t)]
                   [else
...
(define (getExpr text isParen require canBeEnd)
  ...
  (cond [(and (string=? symbol "(") (member symbol require) )
       (set! val (string-append (string-append val symbol) " "))
         (string-append val (getExpr text (+ isParen 1) (list"true" "false" "id" "not" "(") #f))
         ...
         (cond [(and (string=? symbol ")") (member symbol require))
               ...
               [else
                ...
                (cond [(and (string=? symbol "not") (member symbol require) )
                       (set! val (string-append (string-append val symbol) " "))
                       (string-append val (getExpr text isParen (list "not" "true" "false" "0" "id" "(") #f))
                      ...
                      [else
                       ...
                       (cond [(and (string=? symbol "xor")  (member symbol require))
                              (set! val (string-append (string-append val symbol) " "))
                              (string-append val (getExpr text isParen (list "not" "true" "false" "id" "(") #f))
