...
(define error #f)
(define cur_var "")
(define langs (list "or" "xor" "and" "not" "1" "0" "(" ")" "null"))
                                    ...
                                    [else
                                     ...
                                     (cond [(and (string=? symbol "and") (member symbol require))
                                           ...
                                           [else
                                            ...
                                            (cond [(and (string=? symbol "1") (member symbol require) )
                                                   (set! val (string-append (string-append val symbol) " "))
                                                   (string-append val (getExpr text isParen (list "xor" "and" "or" ")") #t))
         
                                                   ]
                                                  [else
                                                   (cond [(and (string=? symbol "0") (member symbol require) )
                                                           (set! val (string-append (string-append val symbol) " "))
                                                          (string-append val (getExpr text isParen (list "xor" "and" "or" ")") #t))
...
(printf "НИКОЛАЕВ НИКИТА СЕРГЕЕВИЧ - ИКБО-13-17 - ПРАКТИКА 5 - ВАРИАНТ 15\n")
(define code_1 "a := ( 1 and 0 or 1 ) ; ")
(define code_2 "a := ( 1 ) ;  c := 1 xor b ")
(define code_3 "a := ( 1 ) ;  c := 0 xor 1 and not a ; g := ( 1 and not a ) ; ")
